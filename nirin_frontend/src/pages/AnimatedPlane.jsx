...
const AnimatedPlane = () => {
  ...
  useEffect(() => {
    const handleScroll = (event) => {
      ...
      setCurrentPoint((prevPoint) => {
        ...
        const angle = calculateAngle(currentX, currentY, nextX, nextY) - 15;

        setAngleDeg(angle - 5);
        api.start({
          xy: [nextX - planeDimensions.width / 2, nextY - planeDimensions.height / 2],
        });
        return nextPoint;
      });
    };
  ...
  }, [currentPoint, isAnimating, api, isActivated]);
  ...
  return (
    <div className="animated__dark-background" ref={containerRef}>
      ...
      <div className="animated-container">
        <svg
          ...
          className="animated__svg-background"
          onClick={handleSvgClick}
        >
          <text x={startPoint.x} y={startPoint.y - 60} className="animated__text">
            DESTINATION
          </text>
          <text x={endPoint.x} y={endPoint.y + 60} className="animated__text">
            CHOICE OF SERVICES
          </text>
          <text x={thirdPoint.x + 150} y={thirdPoint.y} className="animated__text">
            SEND REQUEST
          </text>
          <text x={fourthPoint.x} y={fourthPoint.y + 60} className="animated__text">
            MANAGER&apos;S CALL
          </text>
          {points.map((point, index) => (
            <circle key={index} cx={point.x} cy={point.y} r="11.5344" className="animated__circle" />
          ))}
          <line x1={startPoint.x} y1={startPoint.y} x2={endPoint.x} y2={endPoint.y} className="animated__line" />
          <line x1={endPoint.x} y1={endPoint.y} x2={thirdPoint.x} y2={thirdPoint.y} className="animated__line" />
          <line x1={thirdPoint.x} y1={thirdPoint.y} x2={fourthPoint.x} y2={fourthPoint.y} className="animated__line" />
          <line x1={fourthPoint.x} y1={fourthPoint.y} x2={fourthLineEnd.x} y2={fourthLineEnd.y} className="animated__line" />
          <animated.g
            style={{
              transform: xy.to((x, y) => `translate(${x}px, ${y}px) rotate(${angleDeg}deg)`),
            }}
          >
            <path
              d="M51.1573 33.9182C51.1573 33.9182 68.7302 38.2341 72.1049 42.4395C73.0236 43.5844 71.7789 45.4192 70.3539 45.7582C64.1097 47.2434 49.7239 42.8541 49.7239 42.8541L23.6057 66.2676L19.3621 64.4967L34.1735 38.5104L14.2183 33.827L3.5882 42.2422L0.394673 40.6903L7.20625 30.5062C7.20625 30.5062 4.07529 29.0583 2.7012 27.8299C1.95874 27.1661 2.05099 25.5264 3.01914 25.3014C4.94129 24.8548 8.51586 24.3362 8.51586 24.3362L7.32153 12.5876L11.003 13.7755L15.0323 24.8768L36.182 29.104L36.1549 0.717263L40.4493 1.74936L51.1573 33.9182Z"
              className="animated__plane"
            />
          </animated.g>
        </svg>
      </div>
